var vm=new Vue({
    ...
    methods:{
        ...
        loadMenuTree:function(){
            let that = this;
            AjaxInvk(getDataBaseUrl() + ADMIN_SERVER.MENU_MANAGE_LIST + "?" + objectToGetParams(that.queryParams),"","get",function (res) {
        ...
        saveModule() {
            ...
            that.$refs['menuForm'].validate((valid) => {
                if (valid) {
                    AjaxInvk(getDataBaseUrl() + ADMIN_SERVER.MENU_NODE_ADD,that.menuForm,"post",function (res) {
                        if(res.code == SERVICE_RESPONSE.SUCCESS.CODE){
                            console.log(res);
                            this.$message({
                                type: 'success',
